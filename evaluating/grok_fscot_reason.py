...
from prompting import fw_cot_reason
load_dotenv()
...
output_file = f'evaluating/grok/fscotrs_{eval_model}/{data_name}.csv'
...
example_template = """  Example {0}:
  ...
  CoT: 
{2}
7. Conclusion: explanations and then the answer is {3}
...
for index, row in tqdm(data.iterrows(), total=data.shape[0], desc=eval_model):
    if pd.isna(row[eval_model]):
        ...
        for idx, sample_row in samples_per_label.iterrows():
            few_examples += example_template.format(idx + 1, sample_row['post'], sample_row['gpt_4o_mini'], sample_row[f'is_{args.task_name}'].upper())
        
        filled_prompt = fw_cot_reason.suicide_bi_prompt.format(few_examples, row['post'])
